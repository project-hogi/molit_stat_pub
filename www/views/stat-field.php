<?php $pagePath = __FILE__; ?>
<?php $bodyClass = "layout-page"; ?>
<?php require_once $_SERVER["DOCUMENT_ROOT"] . "/lib.php"; ?>
<?php require "part/app-header.php"; ?>
<?php require "part/header.php"; ?>
<?php require "part/header-sub.php"; ?>
<h2>분야별통계</h2>
<p class="additional">분야별로 세분화된 통계정보를 확인할 수 있습니다.</p>
<dl class="paging">
	<dt class="skip">현재 페이지 경로</dt>
	<dd>
		<a href="/views/index.php">홈</a>
		<a href="stat-field.php">통계마당</a>
		<span>분야별통계</span>
	</dd>
</dl>
<p class="chk">
	<input type="checkbox" checked="checked" /> 전체보기
	<input type="checkbox" /> 국토교통통계
	<input type="checkbox" /> e-나라지표
	<input type="checkbox" /> 행정자료
</p>
<ul class="nav nav-tabs" role="tablist" id="myTab">
	<li role="presentation" class="active col-md-3"><a href="#field-tab1" aria-controls="home" role="tab" data-toggle="tab">국토&#47;도시</a></li>
	<li role="presentation" class="col-md-3"><a href="#field-tab2" aria-controls="profile" role="tab" data-toggle="tab">주택&#47;토지</a></li>
	<li role="presentation" class="col-md-3"><a href="#field-tab3" aria-controls="messages" role="tab" data-toggle="tab">건설&#47;수자원</a></li>
	<li role="presentation" class="col-md-3"><a href="#field-tab4" style="border-right:0px;" aria-controls="settings" role="tab" data-toggle="tab">교통&#47;물류</a></li>
	<li role="presentation" class="col-md-3"><a href="#field-tab5" aria-controls="home" role="tab" data-toggle="tab">항공</a></li>
	<li role="presentation" class="col-md-3"><a href="#field-tab6" aria-controls="profile" role="tab" data-toggle="tab">도로&#47;철도</a></li>
	<li role="presentation" class="col-md-3"><a href="#field-tab7" aria-controls="messages" role="tab" data-toggle="tab">외부통계&#47;기타</a></li>
</ul>
<p class="stat-btn">
	<a href="#" class="active">국토정책</a>
	<a href="#">지리정보</a>
	<a href="#">도시</a>
</p>
<div class="tab-content">
	<div role="tabpanel" class="tab-pane active" id="field-tab1">
		<div class="col-md-6">
			<ul class="stat-list">
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
			</ul>
		</div>
		<div class="col-md-6">
			<ul class="stat-list">
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
			</ul>
		</div>	
	</div>
	<div role="tabpanel" class="tab-pane" id="field-tab2">
		<div class="col-md-6">
			<ul class="stat-list">
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
			</ul>
		</div>
		<div class="col-md-6">
			<ul class="stat-list">
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
			</ul>
		</div>	
	</div>
	<div role="tabpanel" class="tab-pane" id="field-tab3">
		<div class="col-md-6">
			<ul class="stat-list">
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
			</ul>
		</div>
		<div class="col-md-6">
			<ul class="stat-list">
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
			</ul>
		</div>	
	</div>
	<div role="tabpanel" class="tab-pane" id="field-tab4">
		<div class="col-md-6">
			<ul class="stat-list">
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
			</ul>
		</div>
		<div class="col-md-6">
			<ul class="stat-list">
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
			</ul>
		</div>	
	</div>
	<div role="tabpanel" class="tab-pane" id="field-tab5">
		<div class="col-md-6">
			<ul class="stat-list">
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
			</ul>
		</div>
		<div class="col-md-6">
			<ul class="stat-list">
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
			</ul>
		</div>	
	</div>
	<div role="tabpanel" class="tab-pane" id="field-tab6">
		<div class="col-md-6">
			<ul class="stat-list">
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
			</ul>
		</div>
		<div class="col-md-6">
			<ul class="stat-list">
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
			</ul>
		</div>	
	</div>
	<div role="tabpanel" class="tab-pane" id="field-tab7">
		<div class="col-md-6">
			<ul class="stat-list">
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
			</ul>
		</div>
		<div class="col-md-6">
			<ul class="stat-list">
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
				<li>
					<a href="#">[행정] 개발제한구역현황</a>
					<span class="plus"></span>
				</li>
			</ul>
		</div>	
	</div>
</div>


<?php require "part/footer-sub.php"; ?>
<?php require "part/footer.php"; ?>
<?php require "part/app-footer.php"; ?>